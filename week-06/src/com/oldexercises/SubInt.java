...
public class SubInt {
    ...
    public static int[] findMatchingIndexes(int number, int[] array){
        ArrayList<Integer> listOfIntegerToTest = new ArrayList<>();
        for (int i = 0; i < array.length; i++) {
            if (String.valueOf(array[i]).contains(String.valueOf(number))){
                listOfIntegerToTest.add(i);
            }
        }
        int[] arrayToReturn = new int[listOfIntegerToTest.size()];
        for (int i = 0; i < listOfIntegerToTest.size(); i++) {
            arrayToReturn[i] = listOfIntegerToTest.get(i);
        }
